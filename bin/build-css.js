...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isImgJSCoursePackage(file) ?
    `${outputDir}/ImgJSCourse.css` :
    `${outputDir}/style.css`;
...
function isImgJSCoursePackage(file) {
  return path.normalize(file).includes('packages/ImgJSCourse/');
}


async function compileCSS() {
  ({ default: chalk } = await import('chalk'));
  const files = await glob('packages/{,@ImgJSCourse/}*/src/style.scss');
