describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ ImgJSCourse }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(ImgJSCourse.getPlugin('Transloadit').activeAssemblies).every(
            (a: any) => a.pollInterval,
          ),
        ).to.equal(true)

        ImgJSCourse.cancelAll()

        cy.wait(['@delete']).then(() => {
          // eslint-disable-next-line
          // @ts-ignore fix me
          expect(
            Object.values(ImgJSCourse.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ ImgJSCourse }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      ImgJSCourse.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ ImgJSCourse }) => {
      ...
      const { files } = ImgJSCourse.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      ImgJSCourse.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ ImgJSCourse }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(ImgJSCourse.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ ImgJSCourse }) => {
      const { files } = ImgJSCourse.getState()
      ImgJSCourse.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        ImgJSCourseserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'ImgJSCourse-core:3.2.0,ImgJSCourse-transloadit:3.1.3,ImgJSCourse-tus:3.1.0,ImgJSCourse-dropbox:3.1.1,ImgJSCourse-box:2.1.1,ImgJSCourse-facebook:3.1.1,ImgJSCourse-google-drive:3.1.1,ImgJSCourse-instagram:3.1.1,ImgJSCourse-onedrive:3.1.1,ImgJSCourse-zoom:2.1.1,ImgJSCourse-url:3.3.1',
    ...
    cy.get('.ImgJSCourse-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.ImgJSCourse-StatusBar-actions > .ImgJSCourse-c-btn').click()
    cy.wait([
