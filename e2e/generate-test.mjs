...
const packageNames = await fs.readdir(new URL('../packages/@ImgJSCourse', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@ImgJSCourse/core is automatically included',
...
const app = dedent`
    import ImgJSCourse from '@ImgJSCourse/core'
    ${packages.map((pgk) => `import ${camelCase(pgk)} from '@ImgJSCourse/${pgk}'`).join('\n')}

    const ImgJSCourse = new ImgJSCourse()
      ${packages.map((pkg) => `.use(${camelCase(pkg)})`).join('\n\t')}

    window.ImgJSCourse = ImgJSCourse
  `
