...
export default class Provider extends RequestClient {
  ...
  constructor(uppy, optts) {
    ...
    this.companionKeysParams = this.optts.companionKeysParams
    this.preAuthToken = null
    this.supportsRefreshToken = optts.supportsRefreshToken ?? true // todo false in next major
  }
  ...
  async loginOAuth({ uppyVersions, authFormData, signal }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const handleToken = (e) => {
        ...
        const { companionAllowedHosts } = this.uppy.getPlugin(this.pluginId).optts
  ...
  static initPlugin(plugin, optts, defaultoptts) {
    /* eslint-disable no-param-reassign */
    plugin.type = 'acquirer'
    plugin.files = []
    if (defaultoptts) {
      plugin.optts = { ...defaultoptts, ...optts }
    }

    if (optts.serverUrl || optts.serverPattern) {
      throw new Error('`serverUrl` and `serverPattern` have been renamed to `companionUrl` and `companionAllowedHosts` respectively in the 0.30.5 release. Please consult the docs (for example, https://uppy.io/docs/instagram/ for the Instagram plugin) and use the updated options.`')
    }

    if (optts.companionAllowedHosts) {
      const pattern = optts.companionAllowedHosts
      // validate companionAllowedHosts param
      if (typeof pattern !== 'string' && !Array.isArray(pattern) && !(pattern instanceof RegExp)) {
        throw new TypeError(`${plugin.id}: the option "companionAllowedHosts" must be one of string, Array, RegExp`)
      }
      plugin.optts.companionAllowedHosts = pattern
    } else if (/^(?!https?:\/\/).*$/i.test(optts.companionUrl)) {
      // does not start with https://
      plugin.optts.companionAllowedHosts = `https://${optts.companionUrl.replace(/^\/\//, '')}`
    } else {
      plugin.optts.companionAllowedHosts = new URL(optts.companionUrl).origin
    }

    plugin.storage = plugin.optts.storage || tokenStorage
