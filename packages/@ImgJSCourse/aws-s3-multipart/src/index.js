...
export default class AwsS3Multipart extends BasePlugin {
  ...
  constructor (uppy, optts) {
    super(uppy, optts)
    this.type = 'uploader'
    this.id = this.optts.id || 'AwsS3Multipart'
    this.title = 'AWS S3 Multipart'
    this.#client = new RequestClient(uppy, optts)
    ...
    const defaultOptions = {
      ...
      signPart: optts?.getTemporarySecurityCredentials ? this.createSignedURL.bind(this) : this.signPart.bind(this),
      uploadPartBytes: AwsS3Multipart.uploadPartBytes,
      getUploadParameters: optts?.getTemporarySecurityCredentials
        ? this.createSignedURL.bind(this)
        : this.getUploadParameters.bind(this),
      companionHeaders: {},
    }

    this.optts = { ...defaultOptions, ...optts }
    if (optts?.prepareUploadParts != null && optts.signPart == null) {
      this.optts.signPart = async (file, { uploadId, key, partNumber, body, signal }) => {
        const { presignedUrls, headers } = await optts
     ...
     */
    this.requests = this.optts.rateLimitedQueue ?? new RateLimitedQueue(this.optts.limit)
   ...
   *
   * Set `optts.abort` to tell S3 that the multipart upload is cancelled and must be removed.
   * This should be done when the user cancels the upload, not when the upload is completed or errored.
   */
  resetUploaderReferences (fileID, optts = {}) {
    if (this.uploaders[fileID]) {
      this.uploaders[fileID].abort({ really: optts.abort || false })
  ...
  createMultipartUpload (file, signal) {
    ...
    const metadata = getAllowedMetadata({ meta: file.meta, allowedMetaFields: this.optts.allowedMetaFields })
  ...
  async #getTemporarySecurityCredentials (options) {
    ...
    if (this.#cachedTemporaryCredentials == null) {
      ...
      if (this.optts.getTemporarySecurityCredentials === true) {
        this.assertHost('getTemporarySecurityCredentials')
        this.#cachedTemporaryCredentials = this.#client.get('s3/sts', null, options).then(assertServerError)
      } else {
        this.#cachedTemporaryCredentials = this.optts.getTemporarySecurityCredentials(options)
      }
  ...
  getUploadParameters (file, options) {
    ...
    const metadata = getAllowedMetadata({ meta, allowedMetaFields: this.optts.allowedMetaFields, querify: true })
  ...
  #uploadLocalFile (file) {
    return new Promise((resolve, reject) => {
      ...
      const upload = new MultipartUploader(file.data, {
        ...
        getChunkSize: this.optts.getChunkSize ? this.optts.getChunkSize.bind(this) : null,
        ...
        file,
        shouldUseMultipart: this.optts.shouldUseMultipart,
  ...
  #setCompanionHeaders = () => {
    this.#client.setCompanionHeaders(this.optts.companionHeaders)
  }
