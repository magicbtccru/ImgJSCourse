...
describe('AwsS3Multipart', () => {
  ...
  describe('companionUrl assertion', () => {
    it('Throws an error for main functions if configured without companionUrl', () => {
      ...
      expect(() => awsS3Multipart.optts.createMultipartUpload(file)).toThrow(
        err,
      )
      expect(() => awsS3Multipart.optts.listParts(file, optts)).toThrow(err)
      expect(() => awsS3Multipart.optts.completeMultipartUpload(file, optts)).toThrow(err)
      expect(() => awsS3Multipart.optts.abortMultipartUpload(file, optts)).toThrow(err)
      expect(() => awsS3Multipart.optts.signPart(file, optts)).toThrow(err)
    })
  })
  ...
  describe('without companionUrl (custom main functions)', () => {
    ...
    it('Calls the prepareUploadParts function totalChunks / limit times', async () => {
      ...
      expect(
        awsS3Multipart.optts.prepareUploadParts.mock.calls.length,
      ).toEqual(2)
    ...
    it('Calls prepareUploadParts with a Math.ceil(limit / 2) minimum, instead of one at a time for the remaining chunks after the first limit batch', async () => {
      ...
      expect(awsS3Multipart.optts.prepareUploadParts.mock.calls.length).toEqual(10)

      validatePartData(awsS3Multipart.optts.prepareUploadParts.mock.calls[0][1], [1])
      validatePartData(awsS3Multipart.optts.prepareUploadParts.mock.calls[1][1], [2])
      validatePartData(awsS3Multipart.optts.prepareUploadParts.mock.calls[2][1], [3])

      const completeCall = awsS3Multipart.optts.completeMultipartUpload.mock.calls[0][1]
  ...
  describe('MultipartUploader', () => {
    ...
    it('retries uploadPartBytes when it fails once', async () => {
      ...
      expect(awsS3Multipart.optts.uploadPartBytes.mock.calls.length).toEqual(3)
    })
